    ...
    $.ajax({
        ...
        success: function (SalarySystemRes) {
            ...
            for (let item of salarySystemList) {
                ...
                <hr/>
                <label class="">${i}.  編號 : ${item.employeeCode} 姓名 : ${item.name}  薪資日期 : ${item.salaryDate}  底薪 : ${item.salary} 一般加給 : ${item.raisePay}  主管加給 : ${item.managerRaisePay}  逞罰 : ${item.salaryDeduct}  總薪資 : ${item.totalSalary}</label>
